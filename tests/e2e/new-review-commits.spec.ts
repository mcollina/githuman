...
test.describe('New Review - API Integration', () => {
  test('commits API returns paginated response', async ({ request }) => {
    const response = await request.get('/api/git/commits?limit=5')
  ...
  test('commits API supports search parameter', async ({ request }) => {
    const response = await request.get('/api/git/commits?search=add&limit=10')
    expect(response.ok()).toBeTruthy()
  ...
  test('commits API supports pagination with offset', async ({ request }) => {
    // Get first page
    const page1 = await request.get('/api/git/commits?limit=2&offset=0')
    const data1 = await page1.json()

    // Get second page
    const page2 = await request.get('/api/git/commits?limit=2&offset=2')
    const data2 = await page2.json()

    // If there are enough commits, pages should be different
    if (data1.commits.length >= 2 && data2.commits.length >= 1) {
      expect(data1.commits[0].sha).not.toBe(data2.commits[0].sha)
    }
  })

  test('can create review from selected commits', async ({ request }) => {
    // Get a commit SHA
    const commitsResponse = await request.get('/api/git/commits?limit=1')
    const commitsData = await commitsResponse.json()

    if (commitsData.commits.length > 0) {
      const sha = commitsData.commits[0].sha

      // Create review with this commit
      ...
      expect(review.sourceType).toBe('commits')
    }
  })
})
